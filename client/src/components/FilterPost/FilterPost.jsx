...
export default function FilterPost() {
  ...
  return (
    <form className="filter-post" onSubmit={onSubmit}>
...
export function AddTag({ onSubmit, value }) {
  ...
  function toggle(tag) {
    function isSelected(tag) {
      return (
        selectedTags.find((selectedTag) => selectedTag.id === tag.id) === -1
      );
    }
    isSelected(tag)
      ? setSelectedTags(selectedTags.filter(isSelected))
    ...
    <div className="add-tag">
      ...
      <dialog open={isDialogOpen}>
        <div className="dialog-header">
          ...
          <Button type="button" className="button-small" onClick={submit}>
            {" "}
            <X />{" "}
          </Button>
        </div>
        <div className="dialog-body">
          {tags.length == 0 ? (
            <span> Aucun tag à selectionner </span>
          ) : (
          ...
          <Button type="button" onClick={submit} className="button-full">
            {" "}
            Confirmer{" "}
          </Button>
        </div>
